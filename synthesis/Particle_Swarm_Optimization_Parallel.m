...
while err > 1e-2 && itr<max_iteration
    ...
    parfor p=1:Bird_in_swarm
        ...
        availability(p,itr)=feval(Food_availability,parameter,formula,true);
    end
    itrTime=toc;
    fid=fopen(log_file,'a');
    ...
    for i=1:Number_of_quality_in_Bird
        if i==1 || i==3
            value = num2gene(gBest(i));
        else
            value = num2str(floor(gBest(i)));
        end
...
for i=1:Number_of_quality_in_Bird
    if i==1 || i==3
        value = num2gene(optimised_parameters(i));
    else
        value = num2str(floor(optimised_parameters(i)));
    end
    fprintf(fid, 'Parameter %d = %s \n', i, value);
end
